...
public class JogadorServiceTest {
    ...
    class findUser {
        ...
        void findOneCase1() {
            ...
            when(repository.findById(id)).thenReturn(Optional.of(new Jogador()));
        ...
        void findOneCase2() {
            ...
            final Long id = 1L;
            final String message = Codes.JOGADOR_NOT_FOUND_EXCEPTION.getMessage().concat(" For id: ")
                    .concat(id.toString());
    ...
    class createUser {
        ...
        void createCase1() {
            ...
            when(repository.findByUsername(jogador.getUsername())).thenReturn(Optional.empty());
        ...
        @Test
        @DisplayName("Should throw Exception when create jogador.")
        void createCase2() {
            ...
            Assertions.assertEquals(Codes.USERNAME_EXISTS_EXCEPTION.getMessage(), thrown.getMessage());
        }

        @Test
        @DisplayName("Should not create when jogador username is blank.")
        void createCase3() {

            final Jogador jogador = new Jogador("", "senha123");

            when(repository.findByUsername(usernameArgumentCaptor.capture())).thenReturn(Optional.empty());

            jogadorService.create(jogador);

            String username = usernameArgumentCaptor.getValue();

            Assertions.assertEquals(jogador.getUsername(), username);
        }
    }
}
